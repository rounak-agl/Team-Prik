...
class ClickHouseStore:
    ...
    # ── demand from last-year same-day occupancy (DD-01 style) ────────────────
    def ly_demand_score(self, service_number: str, journey_date):
        """0..100 demand proxy = final occupancy of this service on the
        weekday-aligned date last year (journey_date - 364)."""
        from datetime import timedelta
        ly = journey_date - timedelta(days=364)
        rows = self.query(
            '''SELECT countIf(Ticket_Status = 'A') * 100.0 / nullIf(any(total_seats), 0)
               FROM freshbus_operations.bus_ticket_data
               WHERE Service_Number = {sn:String} AND Journey_Date = {jd:Date}''',
            {"sn": service_number, "jd": ly})
        if rows and rows[0][0] is not None:
            return max(0, min(100, round(float(rows[0][0]))))
        return None
